    ...
    def forward(self, x):
        ...
        for i in range(self.nl):   # nl:detect层的数目
            ...
            if self.stride is not None:
                ...
                anchor_grid = torch.cat([self.anchor_grid[i], self.anchor_grid[i], self.anchor_grid[i], self.anchor_grid[i],torch.ones(1, self.na, ny, nx, self.no - 8).to(x[i].device)], dim=4)
                stride = torch.tensor([*[self.stride[i]] * 8, *[1.] * (self.no - 8)]).to(x[i].device)
                grid = torch.cat([self.grid[i], self.grid[i], self.grid[i], self.grid[i], torch.zeros(1, 3, ny, nx, self.no - 8).to(x[i].device)], dim=4)
